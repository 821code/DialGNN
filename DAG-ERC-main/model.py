...
class DAGERC_fushion(nn.Module):
    ...
    def __init__(self, args, num_class):
        ...
        layers = [nn.Linear(1924, args.hidden_dim), nn.ReLU()]
        for _ in range(args.mlp_layers - 1):
            layers += [nn.Linear(args.hidden_dim, args.hidden_dim), nn.ReLU()]
        layers += [self.dropout]
        layers += [nn.Linear(args.hidden_dim, num_class)]
        ...
        self.rnn=nn.GRU(1924,512,2,batch_first=True,bidirectional=True)
        # self.maxpool1 = nn.MaxPool1d(kernel_size=3, ceil_mode=True)
    ...
    def forward(self, features, adj,s_mask,s_mask_onehot, lengths):
        ...
        #--------------------RNN
        # embedded=nn.utils.rnn.pack_padded_sequence(H,lengths.detach().cpu(),batch_first=True,enforce_sorted=False)
        ...
        # --------------------
        # mx_pool_feature=torch.zeros(lengths.size(0),H.size(2))
        # print(H)
        # print(lengths)
        # features=torch.zeros(H.size(0),1924).cuda()
        # for i in range(H.size(0)):
        #     lenth=lengths[i]
        #     t=H[i,:lenth,:]
        #     features[i, :1924] = torch.max(t, dim=0)[0]
            # features[i, :1924] = torch.mean(t, dim=0)
        # print(features)
        # print(features.size())
        # --------------------
        ...
        # --------------------截断
        # H=H.view(H.size(0),-1)
        # features = torch.zeros(H.size(0), 1924*15).cuda()
        # for i in range(H.size(0)):
        #     features[i,:1924*15] = H[i,1924 * 5 : 1924 * 20]
        # --------------------截断
        logits = self.out_mlp(H)
        return logits
