...
def train_or_eval_model(model, loss_function, dataloader,epoch, cuda, args, optimizer=None, train=False):
    ...
    for batch_idx,data in enumerate(dataloader):
        ...
        pred = torch.argmax(log_prob, dim = 1).cpu().numpy().tolist()
        print(pred)
        ...
        if batch_idx % 1 == 0:
            print(f'Time:{datetime.datetime.now().time()},Epoch: {epoch}, BATCH: {batch_idx}, Training Loss:  {round(np.sum(losses) / len(losses), 4)},')
