...
parser.add_argument('--dataset', type=str, default='ECS', help='Dataset string: R8, R52, OH, 20NGnew, MR')
...
parser.add_argument('--hidden_dim',type = int, default = 150, help="The hidden dimension of GCN model")
parser.add_argument('--dropout',type = float, default = 0.5, help="The dropout rate of GCN model")
#parser.add_argument('--dropout',type = float, default = 0.3, help="The dropout rate of GCN model")
...
parser.add_argument('--epochs',type = int, default = 3000, help="Number of maximum epochs")
...
if not args.easy_copy:
    ...
    targetResult = open('targetResult.txt', encoding='utf-8', mode='w')
    predictResult = open('predictResult.txt', encoding='utf-8', mode='w')
    for i in test_result:
        predictResult.write(str(i)+'\n')
    predictResult.close()
    for i in labels[train_size:].cpu():
        targetResult.write(str(i)+'\n')
    targetResult.close()
    times = open('times.txt', encoding='utf-8', mode='w')
    times.write('times:'+str(0)+'\n')
    acc=accuracy_score(labels[train_size:].cpu(),test_result)
    bestACC=acc
    print('ACC',acc)
    print('macF1',f1_score(labels[train_size:].cpu(),test_result,average='macro'))
...
if args.multiple_times:
    ...
    for t in range(args.multiple_times):
        ...
        f1_macro_list.append(f1_score(labels[train_size:].cpu(),test_result,average='macro'))
        acc=accuracy_score(labels[train_size:].cpu(),test_result)
        if(acc>bestACC):
            bestACC=acc
            targetResult = open('targetResult.txt', encoding='utf-8', mode='w')
    ...
    if args.easy_copy:
        ...
        for t in f1_macro_list:
            print("%.4f"%t)
        print("Test F1:",np.round(f1_macro_list,4).tolist())
        print("Mean:%.4f"%np.mean(f1_macro_list))
        print("Std:%.4f"%np.std(f1_macro_list))
